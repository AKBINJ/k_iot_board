 ...
 *   로그인) - "학생증" 발급받기
 *
 *   AccessToken) (복도 돌아다닐 떄 보여주는)  1일짜리 학생증 (학생이 소지)
 ...
 *                      >> 어디는 누구나 들어와도 외고, 어디는 학생만, 어디는 교사만 출입 가능
 *
 ...
 *       - 현재 방식: 서버는 세션 안 쓰고, 매 요청마다 JWT 검사
 ...
 *      generateEmailJwtToken: 이메일 인증 / 비밀번호 재성정 등에 사용
 *
 *  3) 검증 메서드들
 *      isValidToken(String token): 서명 검사 + 만료 시간 체크 >> 문제 없으면 true
 *      getUsernameFromJwt(String token): 토큰 안테 들어있는 username(subject) 꺼냄
 *      getRemainingMillis(String token): 만료까지 남은 시간
 *      removeBearer(String bearerToken): "Bear "까지 제거
 *
 ...
 *      >> SecurityContextHolder 내부에 context에 저장
 *      >> 저장할 데이터 형식이 UsernamePasswordAuthenticationToken 형식
 *
 *  5) 컨트롤러 @AuthenticationPrincipal 등으로 현재 유저 정보 사용 가능
 *
